...
from dragoonterminal.command_flags import parser, is_slideshow
from dragoonterminal.database import Database
from dragoonterminal.filters import Filter

PIPE_PATH = os.environ["HOME"] + "/.dragoon-terminal-pipe" + str(os.getppid())
...
def main(argv):
    ...
    if __name__ != "__main__":
        Filter.filtered_list = [pok for pok in Filter.IMAGE_LIST]
    ...
    if size == 0:
        print("No image matches the specified filters")
        return

    if options.id <= 0:
        # TODO this doesn't account for the current set image and might try
        # TODO to set the same image again (essentially not doing anything)
        target = random.choice(Filter.filtered_list)
    else:
        options.id -= 1
        if len(Filter.IMAGE_LIST) > options.id:
            if len(sys.argv) > 2:
                print("ID has been specified, ignoring all filters.")
            size = 1
            target = Filter.IMAGE_LIST[options.id]
    ...
    if options.verbose or options.dry_run:
        if size == 1:
            print('A single image matches the specified criteria: ')
        if size > Database.MAX_ID:
            print('No image has been filtered...')
        else:
        ...
        print("Total of %d images matched the filters. Chose %s" %
